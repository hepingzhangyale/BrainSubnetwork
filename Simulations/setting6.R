#!/usr/bin/env Rscript
#SBATCH --mem-per-cpu=2g  --time=2:00:00 --mail-type=ALL --mail-user=yisha.yao@yale.edu


load("/gpfs/gibbs/project/zhang_heping/yy634/cluster_gene_association/real_data/chr1/chr1_design.RData")
rm(n_X, n_X_group_sizes)  
alpha <- 100;  opt_lam <- 4500; lam_0 <- 6000;  lam_thred <- 1400;  p <- 200;  del <- 0.05*n/n_q  # tuning lam_0, lam_thred, alpha
...
for (j in 1:p){
  ...
  if (all(grp_est==0)){}else {
    ...
    if (length(grp_ind)==1){ Beta_0[grp_ind,j] <- as.vector(lm(Y[,j] ~ n_s_PC_design[,grp_ind] -1 )$coefficients)} else {
...
repeat{
  ...
  for (j in 1:p){
    ...
    if (length(n_g_ind) ==0){ Xz <- n_s_PC_design; zero_ind <- c(1:n_pc_d) }else{ 
      nonzero_list <- list()
      cov_list <- list()
      for (i in n_g_ind){ 
...
save(n_pc_d, p, n_q, n_PC_group_sizes, Beta, Beta_labels, Beta_cluster_num, true_means,
     ...
     file = "/gpfs/gibbs/project/zhang_heping/yy634/cluster_gene_association/submit2/simulation2/setting6_alpha100_lamthred1400.RData")
