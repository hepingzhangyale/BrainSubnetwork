...
#setting 2
n <- 1500;  d <- 6000;  q <- 800;  p <- 200;  del <- 0.12
alpha <- 800; opt_lam <- 3600; lam_0 <- 5300;  lam_thred <- 1200 # tuning lam_0, lam_thred, alpha=100
...
W <- matrix(0, d, d)
for(i in 1:d){
  for (j in 1:d){ W[i,j] <- 0.2^(abs(i-j)) }
}
...
save(d, p, q, group_sizes, Beta, Beta_labels, Beta_cluster_num, true_means,
     ...
     file = "/gpfs/gibbs/project/zhang_heping/yy634/cluster_gene_association/submit2/simulation2/setting4_alpha800_lamthred1200.RData")
